...
function make_dots(s, n_x, n_y, n, colors, min_radius, radius_scale) {
  var n_dots = 0;
  while (n_dots < n) {
    s.circle(-2 * radius_scale + (n_x + 4 * radius_scale) * Math.random(), -2 * radius_scale + (n_y + 4 * radius_scale) * Math.random(), min_radius + radius_scale * Math.random()).attr({
      stroke: "none",
...
function regenerate() {
  ...
  var leaf_colors = [ scale(.05), scale(.5), scale(.95) ];
  ...
  _.each(_.range(N_LAYERS), function(i) {
    var n_branches = branch_density * (MIN_BRANCHES + 200 * Math.pow((N_LAYERS - (i + .5)) / N_LAYERS, 2.5));
    var branch_width = .5 * Math.pow((i + 1) / N_LAYERS, 2.5);
    var branch_scale = 1 * Math.pow((i + 1) / N_LAYERS, 2);
    var n_dots = leaf_density * (MIN_DOTS + 500 * Math.pow((N_LAYERS - (i + 0)) / N_LAYERS, .5));
    var radius_scale = .5 * Math.pow((i + 1) / N_LAYERS, 2);
    ...
    make_dots(s, N_X, N_Y, n_dots, colors, MIN_RADIUS, radius_scale);
    make_branches(s, N_X, N_Y, n_branches, chroma.mix(sky_color, branch_color, (i + 1) / N_LAYERS, "lab").hex(), branch_width, branch_scale);
  });
}
