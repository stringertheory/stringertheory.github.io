...
function get_tail_length(x, y, max_y, squares) {
  var same_ys = [];
  _.each(_.filter(squares, function(s) {
    return s.x == x;
  }), function(s) {
    same_ys.push(s.y);
  });
  ...
  same_ys.push(max_y);
  same_ys = _.filter(same_ys, function(s) {
    return s > y;
  });
...
function regenerate() {
  ...
  var unique_squares = _.unique(squares, function(s) {
    return s.x + "," + s.y;
  });
  ...
  _.each(unique_squares, function(square) {
    ...
    if (square.y + length < N_Y - 1 && length > 0) {
      _.each(_.range(n_concentric), function(j) {
        s.tailpath(square.x + j / n_concentric / 2, square.y + 1, square.x + 1 - j / n_concentric / 2, square.y + length + 1).attr({
          fill: "white",
          stroke: STROKE_COLOR,
          strokeWidth: STROKE_WIDTH,
    ...
    s.pathline(square.x + 1 / 2, square.y + 1, square.x + 1 / 2, square.y + length + 1).attr({
      fill: "none",
      stroke: STROKE_COLOR,
      strokeWidth: STROKE_WIDTH,
    ...
    _.each(_.range(n_concentric, 0, -1), function(j) {
      s.pathrect(square.x + 1 / 2 - j / n_concentric / 2, square.y + 1 / 2 - j / n_concentric / 2, j / n_concentric, j / n_concentric).attr({
        fill: "none",
        stroke: STROKE_COLOR,
        strokeWidth: STROKE_WIDTH,
